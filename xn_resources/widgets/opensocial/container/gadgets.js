...
gadgets.error.NEED_TOKEN = 'must supply security token';
...
gadgets.IfrGadgetService.prototype.requestNavigateTo = function(view,
  ...
  if (gadget) {
      url = gadget.getNavigationUrl(view, opt_params, gadget);
  }

  if (url) {
    // ignore trailing parameters for canvas switch.
    var qIndex = url.indexOf('?');
    var newUrl = (qIndex == -1) ? url : url.substring(0, qIndex);

    if (document.location.href.indexOf(newUrl) === -1) {
      document.location.href = url;
    }
  }
};

gadgets.IfrGadgetService.getGadgetIdFromModuleId = function(moduleId) {
  // Extract the gadget id from module id. This requires the iframes
  // to have their id at the very end behind a "_" character. 
 ...
 *    "viewParams": a JSON String containing attribute value pairs for this gadget
...
gadgets.Gadget = function(params) {
  ...
  if (!this.secureToken) {
    throw Error(gadgets.error.NEED_TOKEN);
  }
};
...
gadgets.Gadget.prototype.render = function(chrome) {
  if (chrome) {
    this.getContent(function(content) {
      ...
      var i = chrome.getElementsByTagName('iframe')[0];
      i.src = i.getAttribute('_src');
    });
  }
};
...
gadgets.Gadget.prototype.getAdditionalParams = function() {
  return  '&ownerId=' + this.ownerId +
          ...
          '&anchorColor=' + encodeURIComponent(this.anchorColor);
}
...
// This list must be in sync with the list in the container-configuration in the osoc.
gadgets.IfrGadget.prototype.ALLOWED_VIEWS = {'profile' : 'navigateToProfile', 
                                             'home'    : 'navigateToHome', 
                                             'canvas'  : 'navigateToCanvas', 
                                             'preview' : 'navigateToPreview' 
                                            };

gadgets.IfrGadget.prototype.cssClassGadget = '_opensocial-gadget-container';
gadgets.IfrGadget.prototype.cssClassTitleBar = '_opensocial-gadget-titlebar';
gadgets.IfrGadget.prototype.cssClassTitle = '_opensocial-gadget-title';
gadgets.IfrGadget.prototype.cssClassTitleButtonBar = '_opensocial-gadget-title-button-bar';
gadgets.IfrGadget.prototype.cssClassTitleButton = '_opensocial-gadget-title-button';
gadgets.IfrGadget.prototype.cssClassGadgetContent = '_opensocial-gadget-content';
gadgets.IfrGadget.prototype.rpcToken = ((0x7FFFFFFF * Math.random()) | 0);
gadgets.IfrGadget.prototype.rpcRelay = '/html/rpc_relay.html';

gadgets.IfrGadget.prototype.getTitleBarContent = function(continuation) {
  continuation('<div id="' + this.cssClassTitleBar + '-' + this.id +
      '" class="' + this.cssClassTitleBar + '" style="display:none"><span id="' +
...
gadgets.IfrGadget.prototype.setServerUrl = function(url) {
  this.serverUrl_ = url;
};

gadgets.IfrGadget.prototype.getServerUrl = function() {
  return this.serverUrl_;
};

gadgets.IfrGadget.prototype.getMainContent = function(continuation) {
  var iframeId = this.getIframeId();
  gadgets.rpc.setRelayUrl(iframeId, this.serverUrl_ + this.rpcRelay);
  gadgets.rpc.setAuthToken(iframeId, this.rpcToken);
  var el = '<div class="' + this.cssClassGadgetContent + '"><iframe id="' +
      iframeId + '" name="' + iframeId + '" class="' + this.cssClassGadget +
      '" src="about:blank" _src="' + this.getIframeUrl() +
...
gadgets.IfrGadget.prototype.getNavigationUrl = function(view, opt_params, gadget) {

  if (view in this.ALLOWED_VIEWS) {

    url = "/index.php/opensocial/view/" + this.ALLOWED_VIEWS[view] + "?" +
    'ownerId=' + encodeURIComponent(gadget.ownerId) + '&appUrl=' + encodeURIComponent(gadget.specUrl);
...
gadgets.IfrGadget.prototype.getIframeUrl = function() {
   url= this.iframeUrl + '/ifr?' +
      ...
      (this.secureToken ? '&st=' + this.secureToken : '') +
      (this.viewParams ? '&view-params=' +  encodeURIComponent(this.viewParams) : '') +
...
gadgets.Container = function() {
  ...
  this.language_ = 'ALL';
  this.view_ = 'default';
  this.nocache_ = document.location.href.search("[?&]bpc=1(&|$)") > -1 ? 1 : 0;
  // signed max int
