...
dojo.version = {
	...
	major: 0, minor: 3, patch: 1, flag: "-ning",
...
if (!dojo.hostenv.findModule("ning.loader", false)) {
    ...
    (function() {
        var uncompressed = /xn_dojo_uncompressed=true/.test(window.location);
        var have = {ning: {loader: {__module__: true}}};
	    var prefixPatterns = {};
        ...
        var _startPackage = dojo.hostenv.startPackage;
        dojo.hostenv.startPackage = function(packname) {
	        addToModuleHash(have, packname);
			return _startPackage.apply(dojo.hostenv, arguments);
        }

		ning.loader = {
            _pending: {},

            _evalInTopLevel: function(contents) {
                if (!contents) return;
                // eval on IE does *not* run in the top-level context
                window.execScript ? window.execScript(contents) : eval.call(window, contents);
            },

			version: null, // prepended to URL to bust caches

			/**
            * Execute a function only if the given module has NOT already been provided
            *
            * @param moduleName the name of the module to check for
            *
            * @param guardedFunction the function to execute if the module has not been provided
            */
            guard: function(moduleName, guardedFunction) {
            ...
            */
			setPrefixPattern: function(pattern, path) {
				...
				if (patternKernel == String(pattern)) {
					throw "Bogus parameter " + pattern + " passed to ning.loader.setPrefixPattern";
				}
				if (prefixPatterns[pattern]) {
					if (prefixPatterns[pattern] == path) {
						return;
					}
					throw "Pattern " + pattern + " has already been defined by calling ning.loader.setPrefixPattern";
				}
            ...
            /**
            * Loads the given modules and calls a function specified by the last parameter.
            * A best-effort is made not to reload modules that have already been loaded,
            * but whether or not the module is reloaded should not be depended upon
            * (since this may vary depending on the timing of various async requests).
            *
            * @param moduleName1 a required module
            *
            * @param moduleName2 a required module
            *
            * @param moduleName3 a required module
            *
            * @param moduleNameN a required module
            *
            * @param callback the function to execute once all the modules have been loaded, or null to execute synchronously
            */
            require: function() {
				var version = "@NING_RESOURCE_VERSION@_" + (ning.loader.version || (new Date()).valueOf());
	            var maxLength = 800;
				var url = "http://" + window.location.host + "/xn/loader?v=" + version + "&r=";
                var length = arguments.length-1;
                var asyncFunc = false;
                if (arguments[length] instanceof Function || typeof arguments[length] == "function") {
                    asyncFunc = arguments[length];
                }
                else {
                    ++length;
                }
                ...
                if (dojo && dojo.hostenv && dojo.hostenv.modulePrefixes_) {
                    for (var i in dojo.hostenv.modulePrefixes_) {
						...
						if (!matchesPattern) {
	                        url += encodeURIComponent(name) + ":" + encodeURIComponent(dojo.hostenv.modulePrefixes_[i].value) + ",";
						}
                    }
                }
	            ...
	            if (url.length > maxLength) {
		            // we're screwed! only recourse is to fallback to doing it the old-fashione way
		            for (var i = 0; i < length; ++i) {
	            ...
	            if (asyncFunc) {
                    if (djConfig.isDebug) {
                        // use a dynamic script element so firebug shows us code
                        // (is significantly slower than an dojo.hostenv.getText)
                        ...
                        scriptNode.setAttribute("src", url + "&pending=" + id);
                        document.getElementsByTagName("head").item(0).appendChild(scriptNode);
                    }
                    else {
                        dojo.hostenv.getText(url, function(contents) {
                            try {
                                ning.loader._evalInTopLevel(contents);
                                asyncFunc();
                            }
                            catch (e) {
...
dojo.string.substituteParams = function(template /*string */, hash /* object - optional or ... */) {
	...
	return template.replace(/\%\{(\w+)\}/g, function(match, key){
        // http://trac.dojotoolkit.org/ticket/1171 [Tim Williamson 2007-07-27]
...
/**
 * patched to change order of declaration of helper functions so that when it is run through
 * dojo's compressor things don't break. -Brian McCallister, May 8, 2006
 *
 * patched to tell dojo it was loaded if running in a dojo environment. -Brian McCallister, May 8, 2006
 */

if ((dojo != null) && (dojo.provide != null)) dojo.provide("trimpath.template");
...
(function() {               // Using a closure to keep global namespace clean.
	...
	// HACK TO GET DOJO'S OPTIMIZER TO WORK
	var cleanWhiteSpace;
    var scrubWhiteSpace;
	var emitExpression;
    var emitStatement;
	var emitText;
	var emitSectionTextLine;
    var emitSectionText;
	var parse;
 ...
 * - subdomain (string, default value: 'www')
 * - path (string, default value: '/')
 * - query (hash or string, default value: empty string)
 * - ssl (boolean, default value: false)
 *
 * The string that buildUrl() returns is: scheme + "://" + hostport + path + suffix
 * "scheme" is "http" if ssl is false or "https" if ssl is true
 * hostport is subdomain + appropriate domain suffix (.ning.com, .xnq.ningops.net, etc.) + ":port"
 * if the appropriate port is anything but 80 (for ssl=false) or 443 (for ssl=true)
 * If query is a nonempty string, suffix is "?" + query.
 * If query is a nonempty hash, suffix is '?' + the hash converted to key/value pairs
 * Alternatively, if args is a string instead of a hash, it's used as the value for 'subdomain'
 * and all other values have their defaults.
 */
ning.system.buildUrl = function(args) {
	...
	var scheme = args.ssl ? "https" : "http";
	var subdomain = args.subdomain || "www";
	var path = args.path || "/";
	var query = args.query || "";
	if (dojo.lang.isString(query)) {
		if (query.length > 0) {
			query = "?" + query;
		}
	}
		...
		var tmp = "";
		for (var i in query) {
			tmp += dojo.string.substituteParams("%{separator}%{key}=%{value}", {
				separator: tmp.length ? "&" : "?",
				key: encodeURIComponent(i),
				value: encodeURIComponent(query[i])
			});
		}
		query = tmp;
	}
	return dojo.string.substituteParams("%{scheme}://%{subdomain}.%{host}%{port}%{path}%{query}", {
		scheme: scheme,
		subdomain: subdomain,
		path: path,
		query: query,
		host: ning._.domains.base,
		port: args.ssl ?
			  (ning._.domains.ports.ssl == 443 ? "" : ":" + ning._.domains.ports.ssl) :
			  (ning._.domains.ports.http == 80 ? "" : ":" + ning._.domains.ports.http)
	});
}

ning.system.generateCaptcha = function(callback) {
	dojo.io.bind({
		url: "/xn/rest/1.0/captcha",
		load: function(type, data, evt) {
 ...
 * - password: string
 * - remember: boolean
 * - target: url (defaults to current location)
 * - errorTarget: url (defaults to current target)
 * - params: hash of parameters to return with the response (optional)
 *
 * Params must not contain any key that starts with the prefix xn_
 * and from must not start with the prefix xn_
 */
ning.system.signIn = function(args) {
	var query = {};
	for (var i in args.params) {
		query[i] = args.params[i];
	}
	query.xn_user = args.id;
	query.xn_password = args.password;
	query.xn_rememberMe = args.remember;
	query.target = args.target || window.location;
	query.errorTarget = args.errorTarget || query.target;
	try {
		if (!/[:\/?&]/.test(query.target)) {
			query.target = decodeURIComponent(query.target);
		}
		if (!/[:\/?&]/.test(query.errorTarget)) {
			query.errorTarget = decodeURIComponent(query.errorTarget);
		}
	}
	catch (e) {
		// HACK: we used to not url encode target and errorTarget,
		// so try to decode them and assume a failure means that
		// they weren't encoded to begin with...
	}
	window.location = ning.system.buildUrl({
		subdomain: ning.CurrentApp.id,
		ssl: true,
		path: "/xn/identity/1.0/signin",
		query: query
	});
}
...
ning.system.signOut = function(args) {
    ...
    window.location = ning.system.buildUrl({
        subdomain: ning.CurrentApp.id,
		ssl: false,
		path: "/xn/identity/1.0/signout",
		query: args
	});
}

/**
 * accepts keyword args: formNode: form node, target: url, errorTarget: url
 * if target is not supplied, the current location is used
 * if errorTarget is not supplied, the target location is used
 *
 * formNode must have the following input names:
 * - profile_id
 * - profile_email
 * - profile_password
 * - profile_password_confirm
 * - profile_captcha_token
 * - profile_captcha_value
 *
 * formNode may have the following input names:
 * - profile_photo (must be a file upload)
 *
 * any params not prefixed with profile_ are tacked on as params to the target or errorTarget urls
 * errorTarget is *not* passed back (to avoid excessively long urls after multiple errors)
 *
 * in the event of an error, two additional parameters are also passed back: xn_profile_id and xn_profile_email
 * errors are passed back in xn_register_error param(s)
 */
ning.system.signUp = function(args) {
	...
	form.setAttribute("action", ning.system.buildUrl({
		subdomain: ning.CurrentApp.id,
		ssl: true,
		path: "/xn/identity/1.0/signup",
		query: args
	}));
	form.submit();
	// don't worry about ning._.track() -- RegisterRequestHandler sets a cookie which ning.js looks for on the landing page
}

ning.system.cloneApp = function(args) {
	var a = {
		url: "/xn/rest/1.0/application",
		method: "POST",
		mimetype: "text/json",
		encoding: "utf-8",
		load: function(type, data, evt) {
        ...
        error: function(type, data, xmlHttpRequest) {
            if (data && data.type == "unknown" && xmlHttpRequest && xmlHttpRequest.responseText) {
                data = dojo.json.evalJSON(xmlHttpRequest.responseText);
            }
            if (args.failure) {
 ...
 * Two ways of calling:
 * First Way:
 *   ning.system.findProfiles("brian", function(data) { alert(data.results[0]); }, this);
 *
 *   first param is the search term
 *
 *   second is callback to receive the results, which
 *   will only include the first, but will have the total, etc.
 *
 *   third is optional, and is the context (this) to evaluate the callback in
 *
 *
 * Second Way:
 *   ning.system.findProfiles("brian", 10, 13, function(data) {
 *       alert(data.values[0])
 *   }, this);
 *
 *   first param is the search term
 *
 *   second param is the start index for results
 *
 *   third param is the end index for results
 *
 *   fourth param is the callback to receive the results
 *
 *   fifth param is optional and is the context (this) in which to evaluate the callback
 */
ning.system.findProfiles = function(term, from, to, callback, context) {
    // normalize params for either style (see docs)
    if (dojo.lang.isFunction(from)) { callback = from; context = to; from = 0; to = 1; }
    context = context || dj_global;
    ...
    dojo.io.bind({
        url: url,
  		...
  		encoding: "utf-8",
        method: "GET",
        load: function(a, data, b) {
            callback.apply(context, [data.results]);
        }
    })
}
...
ning._ = {
	CurrentClientTime: dojo.date.format(new Date(), "%Y-%m-%dT%H:%M:%S%z"),
    bar: {},
    loaded: false,
    track: function(name, event, source) {
        ning.whenLoaded(function() {
			if (!window['s_xn']) {
				return;
			}
			s_xn.pageName = "Ning - " + ning.CurrentApp.name.replace(/'/gm, "&#039;");
            s_xn.channel = source || "Ningbar";
            s_xn.prop1 = "";
            if (event == "invited") {
                s_xn.prop1 += "Message Sent ";
            }
            s_xn.prop1 = source || "Ningbar";
            var isUserPanel = (ning.Bar.getOpenPanelName() == "user");
            var openPanel = isUserPanel ? "User" : ning.Bar.getOpenPanelLabel();
            if (openPanel) {
                s_xn.prop1 += " - " + openPanel;
            }
            if (name) {
                s_xn.prop1 += " - " + name
            }
            ...
            else if (event == "cloned") {
                s_xn.events = "event3";
            }
            else if (event == "befriended") {
                s_xn.events = "event5";
            }
            else if (event == "contributed") {
                s_xn.events = "event6";
            }
            else if (event == "bought") {
                s_xn.events = "event7";
            }
            void(s_xn.t());
        });
...
(function() {
    var scriptPath = dojo.hostenv.getBaseScriptUri();
    var xnStaticPath = scriptPath.substring(0, scriptPath.indexOf('/js/')) + '/';
    ning._.buildStaticPath = function(path) {
        return xnStaticPath + path;
	}
})();

ning.whenLoaded = function (block) {
	if (ning._.loaded) {
		block();
	}
	else {
		dojo.addOnLoad(block);
	}
}
...
(function() {
    /**
     * Function which is called when the Ningbar is about to be modified
     * Return true to allow the modification, false to prevent it
     * Args should contain the same arguments as specified for addPanel.
     * If function modifies args, the behavior is unspecified.
     */
     ...
     * Function which is called when the Ningbar is about to be modified
     * Return true to allow the modification, false to prevent it
     * Args should contain the same arguments as specified for addDialog.
     * If function modifies args, the behavior is unspecified.
     */
     ...
     * Function which is called when the Ningbar is about to be modified
     * Return true to allow the modification, false to prevent it
     * Args should contain the same arguments as specified for setBrandPanel.
     * If function modifies args, the behavior is unspecified.
     */
...
/**
 * Function which will be called when a tag is viewed
 */
...
(function() {
   ...
   * @nodoc
   */
  var _ajaxBind = function(method, query, content, handlers, context) {
    var ctx = context || dj_global;
  	dojo.io.bind({
  		// IE and Safari done like [ or ] in the request
  		url: query.replace(/\[/g, "%5B").replace(/\]/g, "%5D").replace(/\|/g, "%7C"),
  		method: method,
  		content: content,
  		mimetype: "text/json",
  		encoding: "utf-8",
  		load: function(type, evaldObj) {
  			if (!evaldObj || evaldObj.errors) {
  				if (dojo.lang.isFunction(handlers.error)) {
  					handlers.error.apply(ctx, [evaldObj]);
  				}
  			}
  			else {
  				if (dojo.lang.isFunction(handlers.success)) {
  					handlers.success.apply(ctx, [evaldObj]);
  				}
  			}
  			if (dojo.lang.isFunction(handlers.after)) {
  				handlers.after.apply(ctx, []);
  			}
  		},
  		error: function(type, errorObj, xmlHttpRequest) {
            if (errorObj && errorObj.type == "unknown" && xmlHttpRequest && xmlHttpRequest.responseText) {
                errorObj = dojo.json.evalJSON(xmlHttpRequest.responseText);
            }
  			if (dojo.lang.isFunction(handlers.error)) {
  				handlers.error.apply(ctx, [errorObj]);
  			}
  			if (dojo.lang.isFunction(handlers.after)) {
  				handlers.after.apply(ctx, []);
  			}
  ...
  /**
   * Execute a GET without caching being allowed, specifying utf-8, and forcing JSON
   * return type
   *
   * @param url A URL to execute the bind against
   *
   * @param handlers Contains up to three callbacks:
   *        handlers.error : called when an error occurs (4XX response, etc)
   *        handlers.success: called on a successful bind (2XX response, etc)
   *        handlers.after: always called after either success or failure
   *
   * @param context (optional) : the object which will be "this" on the handler invocatons
   */
  ning.api.get = function(url, handlers, context) {
  	_ajaxBind("GET", url, null, handlers || {}, context);
  }

  /**
   * Execute a POST without caching being allowed, specifying utf-8, and forcing JSON
   * return type
   *
   * @param url A URL to execute the bind against
   *
   * @param content a javascript object which will be used for form parameters,
   *                the property names will be the keys, the values will be values
   *
   * @param handlers Contains up to three callbacks:
   *        handlers.error : called when an error occurs (4XX response, etc)
   *        handlers.success: called on a successful bind (2XX response, etc)
   ...
   * @param context (optional) : the object which will be "this" on the handler invocatons
   */
  ning.api.post = function(url, content, handlers, context) {
  	_ajaxBind("POST", url, content, handlers || {}, context);
  }
...
/**
 * Function which will be called when a profile is viewed
 */
if (typeof(ning.hooks.viewProfile) == "undefined") {
	ning.hooks.viewProfile_panelWidget = null;
	ning.hooks.viewProfile = function(userName, event) {
		if (event) dojo.event.browser.stopEvent(event);
		var requestsPanelExists = ning && ning.Bar && ning.Bar.panelExists("friend-requests");
		var peoplePanelExists = ning && ning.Bar && ning.Bar.panelExists("people");
		if (requestsPanelExists || peoplePanelExists) {
			if (!userName) {
				ning.Bar.open(peoplePanelExists ? "people" : "friend-requests");
			}
			else {
				ning.api.get("/xn/rest/1.0/profile:" + userName, {
					success: function(evaldObj) {
						if (requestsPanelExists) {
							ning._.bar.user.viewProfile(evaldObj.profile);
						}
						else {
							ning._.bar.people.viewProfile(evaldObj.profile);
						}
					},
			...
			// create temporary humphrey
			var setPanelContent = function() {
				ning.loader.require(
					"dojo.widget.*",
					"ning._.bar.people.xnPeopleOnApp",
					"ning._.bar.people.xnPeopleUserPinned", function() {
					if (userName == "__internal_allOnApp") {
						ning.hooks.viewProfile_panelWidget.setContent(dojo.widget.createWidget("xnPeopleOnApp", {
							panelWidget: ning.hooks.viewProfile_panelWidget
						}));
					}
					else {
						ning.api.get("/xn/rest/1.0/profile:" + userName, {
							success: function(evaldObj) {
								ning.hooks.viewProfile_panelWidget.setContent(dojo.widget.createWidget("xnPeopleUserPinned", {
									panelWidget: ning.hooks.viewProfile_panelWidget,
									profile: evaldObj.profile
								}));
							},
							error: function(errorObj) {
			...
			if (ning.hooks.viewProfile_panelWidget) {
				...
				ning.Bar.addPanel({
					name: "view-profile",
					order: 103,
					icon: ning._.buildStaticPath("ningbar/gfx/menu/request.gif"),
					iconSize: {width: 10, height: 13},
					label: "",
					open: function(div, panelWidget) {
						ning.hooks.viewProfile_panelWidget = panelWidget;
						setPanelContent();
					},
					cache: false,
					close: function() {
