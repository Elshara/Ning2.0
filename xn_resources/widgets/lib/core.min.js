/* This file is auto-generated. Do not edit it directly. What's inside: jquery (as x$()), jquery.json, dojo-adapter */
/*
...
{return eval("("+src+")");}
})(jQuery);;window.x$ = jQuery.noConflict();
...
var prefixPatterns={};var compiledPrefixPatterns=[];var have={},provide=dojo.provide;for(var i in dojo._modules){addToModuleHash(have,i);}
dojo.provide=function(mod){addToModuleHash(have,mod);provide.apply(dojo,arguments);}
ning.loader={_pending:{},version:null,setPrefixPattern:function(pattern,path){var patternKernel=String(pattern).replace(/^\/(.*)\/$/,"$1");if(patternKernel==String(pattern)){dojo.raise("Bogus parameter "+pattern+" passed to ning.loader.setPrefixPattern");}
prefixPatterns[patternKernel]=path;compiledPrefixPatterns.push(new RegExp("^"+patternKernel+"$"));},require:function(){var maxLength=800;var url="http://"+window.location.host+"/xn/loader?v="+"6.11.8.1_"+(ning.loader.version||(new Date()).valueOf());var asyncFunc=arguments[arguments.length-1];if("function"!=typeof asyncFunc){dojo.raise("last argument of ning.loader.require() must be a function");}
...
url+="&r="+compressModuleHash(require);url+="&p=";for(var i in dojo._modulePrefixes){var matchesPattern=false;for(var p=0;p<compiledPrefixPatterns.length;++p){var pattern=compiledPrefixPatterns[p];if(pattern.test(i)){matchesPattern=true;break;}}
if(!matchesPattern){url+=encodeURIComponent(i)+":"+encodeURIComponent(dojo._modulePrefixes[i])+",";}}
for(var pattern in prefixPatterns){url+=encodeURIComponent(pattern)+"=>"+encodeURIComponent(prefixPatterns[pattern]);}
if(url.length>maxLength){dojo.raise("Cannot load JS files. URL is too long");}
url+="&h="+compressModuleHash(have);if(url.length>maxLength){url=url.substring(0,maxLength);}
x$.get(url,{},function(contents){try{x$.globalEval(contents);asyncFunc();}
catch(e){setTimeout(function(){dojo.raise(e.message);},0);}});}}})();dojo.provide('dojo.lang');dojo.lang.extend=function(ctor,props){x$.extend(ctor.prototype,props);}
...
if(obj instanceof Array){for(var i=0;i<obj.length;i++){bind(obj[i],evt,cb1,cb2);}}else{bind(obj,evt,cb1,cb2);}},browser:{stopEvent:function(evt){if(evt.stopPropagation)evt.stopPropagation();if(evt.preventDefault)evt.preventDefault();evt.cancelBubble=true;evt.returnValue=false;}}}})();dojo.provide('dojo.io');(function(){var _allowedParams={url:1,method:1,preventCache:1,encoding:1,content:1,formNode:1,sync:1,mimetype:1,load:1,error:1};dojo.io={bind:function(conf){var opts=this.prepareBind(conf);var xhr;if(conf.load)opts.success=function(data,status){conf.load("load",data,xhr)};if(conf.error)opts.error=function(xhr,status,error){conf.error("error",error,xhr)};return xhr=x$.ajax(opts);},prepareBind:function(conf){var o={};for(var i in conf){if(!(i in o)&&!_allowedParams[i])dojo.raise("Unknown parameter `"+i+"'. Use dojo.js instead.");}
...
var opts={url:conf.url,type:conf.method,cache:!conf.preventCache,data:conf.content,async:!conf.sync};switch(conf.mimetype){case"text/javascript":case"text/json":opts.dataType='json';break;case"application/xml":case"text/xml":opts.dataType='xml';break;case'text/html':case'text/plain':opts.dataType='text';break;default:break;}
...
props=props||{};props.fastMixIn=true;var newNode=0;if(!node){newNode=1;node=document.body.appendChild(document.createElement('span'));}
